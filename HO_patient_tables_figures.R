...
# 1. Define the columns used for identifying duplicates
group_cols <- names(matched.final)[which(names(matched.final) == "match"):which(names(matched.final) == "deidentified_patient_id")]

# 2. Group by those columns and keep only groups with duplicates
dup_groups <- matched.final %>%
  group_by(across(all_of(group_cols))) %>%
  filter(n() > 1) %>%
  ungroup()

# 3. Get the remaining columns to compare
other_cols <- setdiff(names(matched.final), group_cols)

# 4. Show differences within each duplicated group
diffs <- dup_groups %>%
  group_by(across(all_of(group_cols))) %>%
  ...
  do({
    ...
    result <- data.frame(ref_row = 1, compare_row = 2:nrow(data),
                         ...
                         Value_1 = as.vector(t(ref)),
                         Value_2 = as.vector(t(comparison)),
                         Different = as.vector(t(differences)))
    result %>% filter(Different)
  })
...
# Compare columns
diff_cols <- names(row1)[row1 != row2]
print(diff_cols)
