...
path_cat_table <- path_cat_hierarchy(micro, hierarchy = "org_group_1") 
...
path_cat_table_matching <- cc.unmatched %>%
  gather(run, group, 'Enteric flora':'Skin flora') %>%
  count(run, group) %>%
  arrange(-n) %>%
  filter(group == "case") %>%
  filter(n >= 300) %>%
...
unmatched_features <- unmatched_features %>%
  select(run, group, PatientID, PatientEncounterID, dept_code, hospitalization_id, room_stay_id, 
         ...
         indiv_score:admit_source_clean, 'prior_Enteric flora':DR_PsA_cp) %>% 
  distinct()
...
# unmatched_features <- unmatched_features %>% filter(age <= 89)
...
matched.final.models <- matched.final.models %>% 
  select(match:group, group_index, group_binary, PatientID, DTS_in_month, DTS_in_year, duration,
         ...
         any_abx_0_60:other_abx_0_60, 'prior_Enteric flora':DR_PsA_cp) %>%
